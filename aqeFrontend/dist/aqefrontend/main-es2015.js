...
let AdminComponent = class AdminComponent {
    constructor(adminService, router, dialog) {
...
let DialogOverviewExampleDialog = class DialogOverviewExampleDialog {
    constructor(dialogRef, data, adminService) {
...
let CreateuserComponent = class CreateuserComponent {
    ...
    openDialog(userDetails) {
        ...
        const dialogRef = this.dialog.open(_create_modal_create_modal_component__WEBPACK_IMPORTED_MODULE_6__["CreateModalComponent"], {
            width: '450px',
    ...
    generateUserAndPass() {
        this.createUserService.generateRandomUserAndPass().subscribe(response => { }, //do nothing
        //do nothing
        error => {
...
let DashboardComponent = class DashboardComponent {
    constructor(topicService, employeeService, requestService, toasterService) {
...
let LoginComponent = class LoginComponent {
    constructor(router, loginservice, registrationService, toastr, toasterService, activatedRoute) {
        this.router = router;
        this.loginservice = loginservice;
...
let RegistrationComponent = class RegistrationComponent {
    constructor(RegistrationService, router, ts) {
        this.RegistrationService = RegistrationService;
        this.router = router;
        this.ts = ts;
        this.isLoading = false;
    ...
    register(username, password, email) {
        this.isLoading = true;
        this.RegistrationService.registerHr(username, password, email).subscribe(response => {
            //do nothing
        }, error => {
            if (error.status == 200) {
                ...
                this.ts.error("Could not Register", "Please contact the office");
            }
            this.isLoading = false;
        });
    }
...
let ResetComponent = class ResetComponent {
    ...
    sendMail(email) {
        this.isLoading = true;
        this.authService.sendMail(email).subscribe(response => {
            // do nothing
            // 
        }, error => {
            if (error.status == 200) {
...
let RegistrationService = class RegistrationService {
    ...
    registerHr(username, password, email) {
        return this.httpClient.post(this.URLprefix + '/register', { username, password, email, "role": "HRM" });
    }
};
...
let UserDetailsComponent = class UserDetailsComponent {
    constructor(changeDetectorRef, media, detService, fb) {
...
let DashboardComponent = class DashboardComponent {
    constructor(dashboardService, dataService, router) {
    ...
    ngOnInit() {
        ...
        this.dashboardService.getActiveTests(userEmail).subscribe((response) => {
            this.activeTestsDataSource = new _angular_material__WEBPACK_IMPORTED_MODULE_3__["MatTableDataSource"](response);
...
let FetchTestComponent = class FetchTestComponent {
    constructor(fetchTestService, dataService, router, toasterService) {
...
let HeaderComponent = class HeaderComponent {
    constructor(location, router, ts) {
...
let ConfirmComponent = class ConfirmComponent {
    constructor(confirmService, dataService, router, ts) {
    ...
    confirmMapping() {
        const testId = uuid__WEBPACK_IMPORTED_MODULE_7__["v4"]();
...
let DashboardComponent = class DashboardComponent {
    ...
    openDialog() {
        ...
        const dialogRef = this.dialog.open(_request_modal_request_modal_component__WEBPACK_IMPORTED_MODULE_8__["RequestModalComponent"], {
            width: '550px',
...
let RequestModalComponent = class RequestModalComponent {
    constructor(dialogRef, data) {
        this.dialogRef = dialogRef;
        this.data = data;
    }
...
RequestModalComponent.ctorParameters = () => [
...
let ThankyouHrComponent = class ThankyouHrComponent {
    constructor() { }
    ngOnInit() {
    }
};
